...
def _symbol_to_degree12(x) -> int:
    ...
    # допускаем только уже нормализованные 0..11 из старых внутренних таблиц,
    # но НЕ используем это для букв A/B/C из coords.csv
    try:
        v = int(s)
        if 0 <= v < 12:
            return v
...
def _parse_octave12(x) -> int:
    """
    Parse octave token from project notation.
    ...
    Important:
    No zero is allowed inside octave token.
    """
    s = str(x).strip().upper()
...
def _octave_to_token12(n: int) -> str:
    """
    Convert positive integer octave back to project no-zero 12-radix token.
...
def _coord_to_note_token(octave: int, degree12: int, subdivisions: tuple[int, ...]) -> str:
    ...
    IMPORTANT:
    The authoritative micro token must come from coords.csv note_token.
    This function is allowed to reconstruct only the exact center token.

    It must NOT invent micro suffixes from raw subdivision integers, because
    raw subdivisions do not carry the required i/a direction semantics.

    Old dangerous behavior produced tokens like:
        9.A'274

    That silently corrupts the resonance ontology. If a non-center micro
    coordinate has no explicit note_token in coords.csv, we fail loudly.
    """
...
def _matrix_csv_shape(path: Path) -> tuple[int, int]:
    """
    Return (probe_count, frame_count) without loading the matrix into RAM.
    """
    probe_count = 0
    frame_count = 0
...
def load_matrix_csv(
    ...
    *,
    ...
    """
    Load huge probe_matrix CSV as disk-backed numpy.memmap.

    This replaces the old full-RAM loader that created:
        list[str] -> list[list[float]] -> ndarray

    The returned object behaves like ndarray for matrix[:, frame_index],
    but the backing storage lives on disk.
    """
    ...
    if not cache_ok:
        ...
        with path.open("r", encoding="utf-8", newline="") as f:
            ...
            for row in reader:
                ...
                # Per-row conversion: only one probe row is materialized at a time.
...
def load_coords_csv(path: str | Path) -> List[ResonanceCoordRow]:
    ...
    with path.open("r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            # Если coords.csv уже содержит корректный token, используем его как SSOT,
            # но дополнительно пересобираем fallback через строгий parser.
            ...
            if existing_token:
                ...
                # Explicit note_token is required for non-center micro coordinates.
                # _coord_to_note_token() will only reconstruct safe center tokens
                # and will fail loudly for ambiguous micro subdivisions.
...
def _find_support_for_harmonic(
    *,
    ...
    for i, coord in enumerate(coords):
        if i >= len(frame_values):
            break

        if abs(coord.frequency_hz - expected_hz) <= tol:
...
def scan_frame_candidates(
    *,
    ...
    for i, energy in enumerate(frame_values):
        energy = float(energy)
...
def iter_frame_candidates(
    *,
    ...
    """
    Yield candidates frame-by-frame.

    This is the streaming entry point for large scans.
    """
...
def infer_candidates(raw_candidates: List[Candidate]) -> InferenceResult:
    ...
    - НЕ выбираем "лучшую" ноту как истину
    - возвращаем candidate set для chain-слоя
    """
