...
def _split_token_micro(token: str) -> Tuple[str, str]:
    """
    Return (coarse_token, micro_suffix_without_apostrophe).
...
def _micro_suffix_to_fraction_semitones(micro: str, *, micro_depth: int = 2) -> float:
    """
    Convert micro suffix into fractional semitone offset.

    The project convention here preserves recursive 12-radix individuality:
        depth 1 -> 12 positions per semitone
        depth 2 -> 144 positions per semitone

    Examples:
        i7  -> +6/12 if interpreted as 0-based digit 7
        a7  -> -6/12
        i27 -> +(1*12 + 6)/144
        a55 -> -(4*12 + 4)/144

    Note:
        We use 0-based digit value internally because token digit "1"
        corresponds to zero offset unit inside a local subdivision layer,
        matching int_to_base12_digit(0) -> "1" in the existing scan/projection code.
    """
...
def _token_to_hz(
    ...
    *,
    ...
    """
    Token -> Hz with micro suffix preserved.

    Old behavior collapsed:
        9.A'i27 == 9.A'-

    This version keeps:
        coarse semitone position + recursive micro fraction.
    """
    ...
    _token_coarse, token_micro = _split_token_micro(token)

    anchor_micro_delta = _micro_suffix_to_fraction_semitones(
        anchor_micro,
        micro_depth=micro_depth,
    )
    token_micro_delta = _micro_suffix_to_fraction_semitones(
        token_micro,
        micro_depth=micro_depth,
    )
...
def _hz_to_token_with_micro(
    ...
    *,
    ...
    """
    Hz -> token projection with recursive micro depth.

    This is local projection, not notation grammar.
    notation12.py remains the token-language SSOT.
    """
    if freq_hz <= 0:
        return ""
    ...
    # Keep fixed-width depth digits to avoid ambiguity in recursive layer.
...
def _pick_token_from_row(row: Dict[str, Any]) -> str:
    """
    Prefer micro-preserved token fields if they exist.

    This allows the module to work with both older cluster files and the newer
    micro/coarse-separated files.
    """
    for key in (
        "anchor_token_micro",
        "note_token_micro",
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=int,
        default=2,
        help="Recursive micro depth for token/hz projection. 2 keeps 144-like individuality.",
    )
