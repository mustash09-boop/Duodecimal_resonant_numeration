...
def _trajectory_link_score(
    *,
    ...
    if probe_overlap > 0:
        score += min(1.2, probe_overlap * 0.25)
        reasons.append(f"probe_overlap_{probe_overlap}")

    prev_suffix_set = {str(item[0]) for item in _parse_json_list(prev_row.get("top_micro_suffixes_json", "")) if isinstance(item, list) and item}
    suffix_set = {str(item[0]) for item in _parse_json_list(row.get("top_micro_suffixes_json", "")) if isinstance(item, list) and item}
    suffix_overlap = _overlap_count(prev_suffix_set, suffix_set)
    if suffix_overlap > 0:
        score += min(0.9, suffix_overlap * 0.18)
...
def _classify_chain(
    *,
    ...
    if trajectory_count >= 3 and observation_frame_count >= 8 and micro_share >= 0.70 and suffix_div <= 4:
        chain_kind = "EXACT_TRAJECTORY_NOTECHAIN"
    elif trajectory_class_counter.get("COARSE_MICRO_DRIFT_TRAJECTORY", 0) >= max(2, trajectory_count // 2):
        chain_kind = "COARSE_DRIFT_TRAJECTORY_CHAIN"
    elif trajectory_class_counter.get("EXACT_MICRO_TRAJECTORY", 0) >= max(2, trajectory_count // 2):
        chain_kind = "MICRO_SHIFT_TRAJECTORY_CHAIN"
    elif coarse_share >= 0.70 and observation_frame_count >= 6:
        chain_kind = "COARSE_BACKBONE_TRAJECTORY_CHAIN"
    else:
        chain_kind = "LOCAL_TRAJECTORY_CHAIN"

    if observation_frame_count >= 12 or trajectory_status_counter.get("CONFIRMED_OBSERVATION_TRAJECTORY", 0) >= 2:
        confirmation = "CONFIRMED_CHAIN_V2"
    elif observation_frame_count >= 6 or trajectory_status_counter.get("PROBABLE_OBSERVATION_TRAJECTORY", 0) >= 2:
        confirmation = "PROBABLE_CHAIN_V2"
    else:
        confirmation = "WEAK_CHAIN_V2"
    return chain_kind, confirmation


def main() -> None:
    ap = argparse.ArgumentParser(
        description="Build micro notechains v2 from early observation trajectories instead of late event/frame-slot structures."
    )
    ...
    ap.add_argument("--min-link-score", type=float, default=4.20)
    args = ap.parse_args()
    ...
    trajectory_to_chain: dict[int, int] = {}
    ...
    for idx, row in enumerate(trajectory_rows, start=1):
        ...
        if best_chain_id and best_score >= float(args.min_link_score):
            ...
            trajectory_to_chain[trajectory_id] = chain_id
    ...
    for chain_id in sorted(chain_trajectory_ids):
                ...
                chain_frame_rows.append(
                    {
                        ...
                        "observation_kind": "trajectory_chain_observation",
                    }
                )
        ...
        chain_rows.append(
            {
                "chain_id": chain_id,
                "source_mode": "MICRO_TRAJECTORY_TO_CHAIN_V2",
    ...
    Path(args.out_meta_json).write_text(
        json.dumps(
            {
                "stage": "micro_notechain_builder_v2",
                "source_mode": "MICRO_TRAJECTORY_TO_CHAIN_V2",
                "inputs": {
