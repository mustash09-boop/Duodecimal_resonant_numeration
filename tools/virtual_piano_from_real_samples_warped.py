...
def _build_mapper(anchor_rows: list[dict[str, str]], total_real_frames: int):
    midi_frames = [_safe_float(r.get("midi_start_frame60")) for r in anchor_rows]
    real_frames = [_safe_float(r.get("real_anchor_frame")) for r in anchor_rows]
    if not midi_frames:
        raise SystemExit("No anchors to build mapping")

    if len(midi_frames) == 1:
        slope_left = slope_right = 1.0
    else:
        slope_left = (real_frames[1] - real_frames[0]) / max(1e-9, (midi_frames[1] - midi_frames[0]))
        slope_right = (real_frames[-1] - real_frames[-2]) / max(1e-9, (midi_frames[-1] - midi_frames[-2]))

    def map_frame(midi_frame: float) -> float:
        if midi_frame <= midi_frames[0]:
            return real_frames[0] + (midi_frame - midi_frames[0]) * slope_left
        if midi_frame >= midi_frames[-1]:
            return real_frames[-1] + (midi_frame - midi_frames[-1]) * slope_right
        return float(np.interp(midi_frame, midi_frames, real_frames))

    meta = {
        "anchor_count": len(anchor_rows),
        "first_anchor_midi": midi_frames[0],
        "first_anchor_real": real_frames[0],
        "last_anchor_midi": midi_frames[-1],
        "last_anchor_real": real_frames[-1],
        "slope_left": slope_left,
        "slope_right": slope_right,
        "total_real_frames": total_real_frames,
    }
    return map_frame, meta


def main() -> None:
    ap = argparse.ArgumentParser(description="Build a virtual piano track warped into real performance time.")
    ...
    ap.add_argument("--out_fitted_virtual_piano_wav", required=True)
    ap.add_argument("--out_fitted_preview_wav", required=True)
    ...
    anchor_rows = _read_csv(Path(args.anchor_csv))
    ...
    total_real_frames = total_samples / sr * FRAME_RATE

    map_frame, mapper_meta = _build_mapper(anchor_rows, total_real_frames=int(total_real_frames))
    ...
    for idx, row in enumerate(piano_rows, start=1):
        ...
        midi_start_frame = _safe_float(row.get("start_frame60"))
        midi_end_frame = _safe_float(row.get("end_frame60"))
        real_start_frame = map_frame(midi_start_frame)
        real_end_frame = max(real_start_frame + 1.0, map_frame(midi_end_frame))
        real_start_sec = real_start_frame / FRAME_RATE
        real_end_sec = real_end_frame / FRAME_RATE
        mapped_duration_sec = max(0.0, real_end_sec - real_start_sec)
        overlay_duration_sec = min(len(sample_audio) / sr, mapped_duration_sec + args.release_pad_sec)

        take_samples = max(1, min(len(sample_audio), int(round(overlay_duration_sec * sr))))
        start_sample = max(0, int(round(real_start_sec * sr)))
        ...
        plan_rows.append(
            {
                ...
                "midi_start_frame60": f"{midi_start_frame:.6f}",
                "midi_end_frame60": f"{midi_end_frame:.6f}",
                "real_start_frame": f"{real_start_frame:.6f}",
                "real_end_frame": f"{real_end_frame:.6f}",
                "real_start_sec": f"{real_start_sec:.9f}",
                "real_end_sec": f"{real_end_sec:.9f}",
                "velocity": str(vel),
    ...
    # Piano-only windows in original score, used as a stable calibration zone.
    ...
    for row in midi_rows:
        ...
        if end <= start:
            continue
        if str(row.get("track_name", "")).startswith("Piano"):
    ...
    with Path(args.out_plan_csv).open("w", encoding="utf-8", newline="") as fh:
        fields = list(plan_rows[0].keys()) if plan_rows else [
            "row_id", "track_name", "note12", "real_start_sec", "real_end_sec", "sample_wav_path"
        ]
    ...
    summary_lines = [
        "WARPED VIRTUAL PIANO FROM REAL SAMPLES",
        "=" * 72,
        ...
        f"anchor_count: {mapper_meta['anchor_count']}",
