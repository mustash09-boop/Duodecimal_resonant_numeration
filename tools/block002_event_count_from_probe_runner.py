...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Run the lighter Block002 event-count pipeline from existing probe_* artifacts: "
            "candidate inference, micro clustering, micro families, and excitation-first event synthesis. "
            "This avoids classic note-tail evaluation and focuses on event counts and event structure."
        )
    )
    ...
    _run_step(
        title="1. Candidate inference",
        module="music12.blocks.Block002_pipeline.resonance_candidate_inference_cli",
        tag=f"{prefix}_candidate_inference_eventcount_v1",
        module_args=[
            ...
            *(["--matrix_cache_dir", matrix_cache_dir] if matrix_cache_dir else []),
            *(["--resume_if_possible"] if args.resume_candidate_inference else []),
        ],
    ...
    _run_step(
        title="2. Micro candidate clustering",
        module="music12.blocks.Block002_pipeline.micro_candidate_cluster_cli",
        tag=f"{prefix}_micro_clusters_eventcount_v1",
        module_args=[
    ...
    _run_step(
        title="3. Micro harmonic families",
        module="music12.blocks.Block002_pipeline.micro_harmonic_family_builder_cli",
        tag=f"{prefix}_micro_families_eventcount_v1",
        module_args=[
    ...
    if midi_meta_json:
        excitation_args.extend(["--midi-meta-json", midi_meta_json])

    _run_tool_script(
        title="4. Excitation-first event pipeline",
        script_path=project_root / "tools" / "block002_excitation_runner.py",
        script_args=excitation_args,
        project_root=project_root,
        python_exe=python_exe,
    )

    print()
    print("BLOCK002 EVENT-COUNT FROM PROBE COMPLETE")
